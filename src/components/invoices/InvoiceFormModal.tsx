...
const emptyRow: InvoiceItemInput = {
  item_type: "service",
  service_id: null,
  product_id: null,
  quantity: 0,
...
const InvoiceFormModal = ({ open, onClose }: Props) => {
  ...
  const patients =
    patientData?.data ??
    (Array.isArray(patientData) ? (patientData as any) : []);
  ...
  const staff =
    staffData?.data ??
    (Array.isArray(staffData) ? (staffData as any) : []);
  ...
  const services =
    serviceData?.data ??
    (Array.isArray(serviceData) ? (serviceData as any) : []);
  ...
  const products =
    itemData?.data ??
    (Array.isArray(itemData) ? (itemData as any) : []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutate({
      ...
      doctor_id:
        requiresDoctor && form.doctor_id
          ? Number(form.doctor_id)
          : null,

      nurse_id:
        allowsNurse && form.nurse_id
          ? Number(form.nurse_id)
          : null,
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <SelectField
            ...
            onChange={(e) =>
              setForm({
                ...form,
                type: e.target.value as Invoice["type"],
              })
            }
            options={INVOICE_TYPES}
          />
          ...
          <SelectField
            ...
            onChange={(e) =>
              setForm({
                ...form,
                payment_method:
                  e.target.value as Invoice["payment_method"],
              })
            }
            options={PAYMENT_METHODS}
          />
          ...
          {requiresDoctor && (
            <SelectField
              ...
              options={doctors.map((doctor: Staff) => ({
                value: doctor.id,
                label: doctor.name,
              }))}
            />
          ...
          {allowsNurse && (
            <SelectField
              ...
              options={nurses.map((nurse: Staff) => ({
                value: nurse.id,
                label: nurse.name,
              }))}
            />
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            {rows.map((row, idx) => (
              <div
                key={idx}
                className="grid grid-cols-12 items-end gap-2 rounded-lg border border-ink/10 p-3"
              >
                {/* Item Type */}
                <div className="col-span-6 sm:col-span-3">
                  <SelectField
                  ...
                  {row.item_type === "service" ? (
                    <SelectField
                      ...
                      options={services.map(
                        (service: Service) => ({
                          value: service.id,
                          label: `${service.name} — ${service.price} ج.م`,
                        })
                      )}
                    />
                  ) : (
                    <SelectField
                      ...
                      options={products.map(
                        (product: Item) => ({
                          value: product.id,
                          label: `${product.name} — ${product.selling_price} ج.م`,
                        })
                      )}
                    />
                  )}
