...
interface Props {
  ...
  onViewProfile?: (p: Patient) => void; // ✅ جديد
  /** رقم أول صف في الصفحة الحالية (لو عندك Pagination من السيرفر)، افتراضيًا 1 */
  startIndex?: number;
}
...
const PatientsTable = ({
  ...
  const columns: Column<Patient>[] = [
    ...
    { header: "الهاتف", accessor: (r) => <span dir="ltr">{r.phone}</span> },
    {
      ...
      accessor: (r) => (
        <StatusBadge
          label={labelOf(GENDER_OPTIONS, r.gender)}
          tone={r.gender === "male" ? "primary" : "coral"}
        />
      ),
    },
