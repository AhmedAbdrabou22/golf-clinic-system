...
/**
 * لو عقد موجود (تعديل) → نحدد النمط تلقائيًا عشان يظهر الفورم كامل على طول.
 * لو عقد جديد → نرجّع "" عشان نجبر المستخدم يختار النمط الأول قبل ما نعرض أي حاجة.
 */
const detectDoctorMode = (contract: Contract | null): DoctorMode | "" => {
  if (!contract) return "";
  if (contract.service_commissions && contract.service_commissions.length > 0) return "fixed_per_service";
...
const ContractFormModal = ({ open, onClose, contract }: Props) => {
  ...
  // لو نوع العقد اتغير لطبيب في عقد جديد، نفضّي النمط عشان يجبره يختار الأول
  const handleContractTypeChange = (value: ContractType) => {
    setForm({ ...form, contract_type: value });
    if (value === "doctor") {
      setDoctorMode(isEdit && contract?.contract_type === "doctor" ? doctorMode : "");
    }
  };

  // هل نعرض باقي الفورم؟
  // - أي نوع غير "طبيب": يظهر على طول بعد اختيار النوع
  // - "طبيب": لازم يختار نمط العمولة الأول
  const showRest = !!contractType && (contractType !== "doctor" || doctorMode !== "");
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? "تعديل العقد" : "عقد جديد"} width="lg">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        {/* 1) نوع العقد — دايمًا أول حاجة تظهر لوحدها */}
        <SelectField
          label="نوع العقد"
          name="contract_type"
          required
        ...
        {/* 2) لو طبيب ولسه ما اختارش النمط → يظهر السلكت دا بس وبس، ولا حاجة تانية */}
        {contractType === "doctor" && doctorMode === "" && (
          <SelectField
            label="نمط احتساب عمولة الطبيب"
            name="doctor_mode"
            required
            ...
            onChange={(e) => setDoctorMode(e.target.value as DoctorMode)}
            options={DOCTOR_COMMISSION_MODES.map((m) => ({ value: m.value, label: m.label }))}
            placeholder="اختر نمط الاحتساب"
          />
        )}

        {/* 3) باقي الفورم بالكامل — يظهر بعد ما ينوع العقد (غير طبيب) أو بعد ما يختار الطبيب نمط العمولة */}
        {showRest && (
          <>
            {/* لو طبيب: نعرض نفس سلكت النمط هنا تاني، عشان يقدر يغيّره، مع تفاصيله */}
            {contractType === "doctor" && (
              <div>
                <SelectField
                  label="نمط احتساب عمولة الطبيب"
        ...
        <div className="mt-2 flex gap-3">
          ...
          <button type="submit" disabled={isLoading || !contractType || !showRest} className="btn-primary flex-1">
            {isLoading ? "جاري الحفظ..." : "حفظ العقد"}
          </button>
        </div>
      </form>
