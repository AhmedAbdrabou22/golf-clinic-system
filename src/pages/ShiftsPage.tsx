...
//       <p className="mt-2 font-display text-2xl font-extrabold text-primary-600">
//   <ShiftTimer startTime={activeShift.start_time} />
// </p>
...
const ShiftsPage = () => {
  ...
  return (
    <div>
      <PageHeader
        title="الشفتات"
        subtitle="فتح وإغلاق شفتات العمل اليومية"
      />
      ...
      {activeShift ? (
        <div className="card mx-auto max-w-md p-6 text-center">
          ...
          <h2 className="font-display text-lg font-bold text-ink">
            يوجد شفت مفتوح حالياً
          </h2>

          <p className="mt-1 text-sm text-ink/50">
            الرصيد الافتتاحي:{" "}
            <span className="font-bold text-ink">
              {activeShift.initial_balance} ج.م
            </span>
          </p>

          <button
            onClick={() => setCloseOpen(true)}
            className="btn-danger mt-5 w-full py-3"
          >
            <FiStopCircle size={18} />
            إغلاق الشفت
          </button>

          <p className="mt-3 font-display text-2xl font-extrabold text-primary-600">
            <ShiftTimer startTime={activeShift.start_time} />
          </p>
        </div>
      ) : (
